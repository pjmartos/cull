...
// TestNG provider. Never miss the impacted test.
assert calcRuns >= 2 :
    "CalcTest must re-run after the Calc change under TestNG; saw ${calcRuns}\n${log}"
...
// the warm run, proving CullTestngListener attributed the runtime class
...
assert otherRuns < calcRuns :
    "TestNG selection precision violated: an unaffected test (OtherTest, " +
